...
/* Force dark text color inside the input box */
...
# ── Session state init ────────────────────────────────────────────────────────
# ── Handle chip prefill BEFORE topic_input widget is instantiated ─────────────
if st.session_state.get("_topic_chip"):
    st.session_state.topic_input = st.session_state._topic_chip
    st.session_state._topic_chip = ""

for key in ("results", "running", "done", "_topic_chip"):
...
if st.session_state.running and not st.session_state.done:
    ...
    # ── Step 1: Search (runs first, then reruns page to show ✓ Done) ──
    if "search" not in results:
        ...
        st.rerun()  # refresh → shows Step 1 as Done, starts Step 2
    ...
    # ── Step 2: Reader ──
        ...
        with st.spinner("📄  Reader Agent is scraping top resources…"):
            ...
            st.session_state.results = dict(results)
        st.rerun()  # refresh → shows Step 2 as Done, starts Step 3
    ...
    # ── Step 3: Writer ──
        ...
        with st.spinner("✍️  Writer is drafting the report…"):
            ...
            st.session_state.results = dict(results)
        st.rerun()  # refresh → shows Step 3 as Done, starts Step 4
    ...
    # ── Step 4: Critic ──
        ...
        st.rerun()  # refresh → shows all 4 steps Done + final results
